...
const LEGAL_DISCLAIMER = 'IMPORTANT: This analysis is provided for informational and educational purposes only and does not constitute formal legal advice. No attorney-client relationship is created. Consult a licensed attorney before signing or relying on binding contracts.';
class GeminiService {
    static instance;
    client = null;
    modelName;
    ...
    constructor() {
        this.modelName = env_1.env.GEMINI_MODEL || 'gemini-3.8-flash';
        if (env_1.env.GEMINI_API_KEY && env_1.env.GEMINI_API_KEY.trim() !== '' && env_1.env.GEMINI_API_KEY !== 'your_gemini_api_key_here') {
            try {
                this.client = new generative_ai_1.GoogleGenerativeAI(env_1.env.GEMINI_API_KEY);
            }
            catch (err) {
                console.warn('[GeminiService] Failed to initialize GoogleGenerativeAI client:', err.message);
            }
        }
    }
    static getInstance() {
     ...
     */
    async generateJSON(systemPrompt, userPrompt, fallbackFn) {
        ...
        for (const m of candidateModels) {
            try {
                ...
                console.warn(`[GeminiService] Model ${m} attempt failed (${err.message}).`);
            }
        }
        ...
        throw new Error(`GenAI generation failed across models (${candidateModels.join(', ')}): ${lastError?.message || 'Unknown error'}`);
    }
    /**
     * Evaluate semantic delta between a contract clause and market-standard benchmark.
     */
    async evaluateSemanticDelta(clauseText, benchmarkText, clauseType, persona = 'All') {
...
${benchmarkText}`;
     ...
     */
    async generateCounterDraft(clauseText, benchmarkText, clauseType, issues) {
        const fallbackFn = () => {
            return {
...
${issues}`;
     ...
     */
    async generateGotchas(flaggedClauses, persona) {
...
${JSON.stringify(flaggedClauses, null, 2)}`;
     ...
     */
    async compareContracts(clausesV1, clausesV2) {
        const fallbackFn = () => ({
            overallSummary: 'Version 2 introduces significant rebalancing, establishing mutual liability caps, standard payment terms, and clear IP carve-outs.',
...
${JSON.stringify(clausesV2, null, 2)}`;
    ...
    async answerGroundedQuestion(question, clauses, documentText) {
        const fallbackFn = () => {
            ...
            const matched = clauses && clauses.length > 0
                ? clauses.find((c) => qLower.includes(c.type.toLowerCase()) ||
                    c.text.toLowerCase().split(' ').some((w) => w.length > 4 && qLower.includes(w))) || clauses[0]
                : null;
            if (matched) {
                return {
            ...
            return {
                answer: 'I am ready to analyze your contract. Please select or paste a contract into LexTrace AI, and ask any specific question about your clauses, obligations, or risks.',
...
${JSON.stringify(clauses, null, 2)}`;
     ...
     */
    async generateLawyerDossier(title, persona, scoredClauses) {
        const highRisks = scoredClauses.filter((c) => c.riskLevel !== types_1.RiskLevel.Standard);
        const fallbackFn = () => {
...
${JSON.stringify(highRisks, null, 2)}`;
     ...
     */
    mockSemanticDelta(clauseText, clauseType, persona) {
        ...
        if (hasAggressive) {
            return {
                riskLevel: types_1.RiskLevel.Unfavorable,
                riskScore: 25,
            ...
            return {
                riskLevel: types_1.RiskLevel.Caution,
                riskScore: 60,
            ...
            return {
                riskLevel: types_1.RiskLevel.Standard,
                riskScore: 90,
