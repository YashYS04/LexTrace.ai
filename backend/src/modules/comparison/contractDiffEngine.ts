...
export class ContractDiffEngine {
   ...
   */
  public static computeWordDiff(text1: string, text2: string): DiffSegment[] {
  ...
  public async compare(
    doc1Name: string,
    clauses1: ParsedClause[],
    doc2Name: string,
    clauses2: ParsedClause[]
  ): Promise<ComparisonResult> {
    const clauseComparisons: ClauseComparison[] = [];
    let totalModifications = 0;
    let highRiskShifts = 0;

    const map1 = new Map<string, ParsedClause>();
    for (const c of clauses1) {
      map1.set(c.clauseType, c);
    }

    const map2 = new Map<string, ParsedClause>();
    ...
    const clausesToAnalyzeV1: Array<{ index: number; type: string; text: string }> = [];
    const clausesToAnalyzeV2: Array<{ index: number; type: string; text: string }> = [];
    ...
    return {
      comparisonId: Hasher.generateId('cmp'),
      doc1Name,
      doc2Name,
